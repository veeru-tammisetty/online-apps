document.addEventListener("DOMContentLoaded", function () {
  ...
  document.title = "Nova Retail — " + p.name;
  ...
  window.novaAdobe && novaAdobe.productView({id:id,name:p.name,price:p.price});
  document.getElementById("add-product").onclick = function () {
    var cart = JSON.parse(localStorage.getItem("novaCart") || "[]");
    ...
    window.novaAdobe && novaAdobe.addToCart({id:id,name:p.name,price:p.price},1);
    alert("Added to cart");
